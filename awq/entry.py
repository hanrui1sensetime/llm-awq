...
parser.add_argument('--q_backend',
                    type=str,
                    default="fake",
                    choices=["fake", "real"])
# save/load real quantized weights
parser.add_argument('--dump_quant',
                    type=str,
                    default=None,
                    help='save quantized model')
parser.add_argument('--load_quant',
                    type=str,
                    default=None,
                    help='load quantized model')
# apply/save/load awq
parser.add_argument('--run_awq',
                    action='store_true',
                    help="perform awq search process")
parser.add_argument('--dump_awq',
                    type=str,
                    default=None,
                    help="save the awq search results")
parser.add_argument('--load_awq',
                    type=str,
                    default=None,
...
def build_model_and_enc(model_path):
    ...
    elif "llama" in config.__class__.__name__.lower():
        enc = LlamaTokenizer.from_pretrained(model_path)
    else:
    ...
    if args.load_quant:  # directly load quantized weights
        ...
        if args.w_bit is not None:
            if args.q_backend == "fake":
                ...
                pseudo_quantize_model_weight(model,
                                             w_bit=args.w_bit,
                                             q_config=q_config)
...
def main():
    ...
    if args.tasks is not None:
        ...
        lm_eval_model = LMEvalAdaptor(args.model_path, model, enc,
                                      args.batch_size)
